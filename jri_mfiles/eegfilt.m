...
%
%   ***JRI modified to take out all the printfs
%
% Usage:
...
use_filtfilt = 1; %JRI switch between filtfilt or filter
...
filtorder = 2*ceil(filtorder/2); %ensure filtorder is even
% *** JRI added 'h' to avoid problems w/ odd filter order--alt, ensure filtorder is even
...
for e = 1:epochs                % filter each epoch, channel 
    for c=1:chans
        if use_filtfilt,
            ...
            smoothdata(c,(e-1)*epochframes+1:e*epochframes) ...
                = filter(filtwts,1,data(c,(e-1)*epochframes+1:e*epochframes));
        end
    end
end
